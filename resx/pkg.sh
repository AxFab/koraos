...
function read_store {
    ...
    parse_yaml "$NM" | grep "$SYSOS"_"$ARCH" | grep -v '_sha2' | sed "s/_"$SYSOS"_"$ARCH".*=.*//" | sort -u
}
...
# Initialize tools
TARGET=`$SCRIPT_HOME/make/host.sh $TARGET`
...
case "$COMMAND" in
    ...
    'help'|'')
        ...
        echo "With options:"
        echo "    -u  --update   Update the store index file"
        echo ""
        echo ""
